...
describe('UnicodeData', function() {
	describe('#get', function() {
		it('should have LATIN CAPITAL LETTER A at index 0x41', function() {
			UnicodeData.promise().should.eventually.match(() => {
				var A = UnicodeData.get(0x41);
				A.should.be.ok();
				...
				var a = UnicodeData.get(A.lowercase);
				a.should.be.ok();
	...
	describe('#find', function() {
		it('should be null if unique term not found', function() {
			UnicodeData.promise().should.eventually.match(() => {
				should(UnicodeData.find('blargle', true)).be.null();
				return true;
			});
		});
		it('should be empty if term not found and not unique', function() {
			UnicodeData.promise().should.eventually.match(() => {
				var m = UnicodeData.find('blargle');
				m.should.be.array();
				m.should.be.empty();
				return true;
			});
		...
		it('should find filtered match', function() {
			UnicodeData.promise().should.eventually.match(() => {
				var c = UnicodeData.find((x) => x.charCode === 0x1F577, true);
				c.should.be.ok();
	...
	describe('#utf8', function() {
		...
		it('should encode > U+FFFF', function() {
			UnicodeData.promise().should.eventually.match(() => {
				var spider = UnicodeData.get(0x1F577);
				spider.should.be.ok();
	...
	describe('#utf16', function() {
		it('should encode < U+FFFF', function() {
			UnicodeData.promise().should.eventually.match(() => {
				...
				var ellipses = UnicodeData.get(0x2026);
